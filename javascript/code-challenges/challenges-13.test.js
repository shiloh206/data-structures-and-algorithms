...
const firstLetters = (arr) => {
  let firstLetterArr = [];
  arr.forEach(element => {
    firstLetterArr.push(element[0].charAt(0));
  });
...
const standardizePhoneNumbers = (arr) => {
  ...
  let regex = /\D+/g;
  arr.forEach(element => {
    cleanedNumbers.push(element.replaceAll(regex, ''));
  });
...
//recieved TA help for this beautiful one liner. I didn't even think to use the split method
