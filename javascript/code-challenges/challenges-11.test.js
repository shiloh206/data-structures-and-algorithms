...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((total, innerArr) => {
    return total + innerArr.reduce((sum, num) => {
      return sum + num;
    }, 0);
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(arr => {
    return arr.filter(val => typeof val === 'number' && val % 5 === 0).map(nums => Math.pow(2, nums));
  })
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shortestSoFar, currentChar) => {
    return Number(currentChar.height) < Number(shortestSoFar.height) ? currentChar : shortestSoFar;
  }).name;
};
